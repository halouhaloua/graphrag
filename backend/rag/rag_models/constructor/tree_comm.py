...
class FastTreeComm:
    def __init__(self, graph, struct_weight=0.3, config=None, chunks_data=None):
        ...
        :param chunks_data: Dict of {chunk_id: chunk_text} for enriching community summaries
        """
    ...
    def _get_community_chunk_texts(self, members, max_chunks=3, max_chars=500):
        ...
        for node in members:
            ...
            if chunk_id and chunk_id in self.chunks_data:
                entry = self.chunks_data[chunk_id]
                raw = entry if isinstance(entry, str) else entry.get("chunk", "")
    ...
    def _build_batch_prompt(self, community_batch):
        ...
        for comm_id, members in community_batch:
            ...
            source_text = self._get_community_chunk_texts(members)
