...
from online_dev.page_manager.schema import (
    PageImportIn,
    PageMetaCreateIn,
    PageMetaListOut,
    PageMetaOut,
...
from online_dev.page_manager.service import PageService, PageServiceException
...
def _build_page_list_out(page, application_name: str = "", application_code: str = "") -> dict:
    """构建页面列表输出"""
    return {
        "id": str(page.id),
        "application_id": page.application_id,
...
async def list_pages(
    ...
    """分页查询页面列表（自动应用数据权限）"""
    result = await PageService.list_with_data_scope(
        db=db,
        page=page,
        page_size=page_size,
    ...
    # 批量查询应用名称和编码
    items = result["items"]
    app_ids = list({p.application_id for p in items if p.application_id})
    app_info_map = {}
    if app_ids:
        app_result = await db.execute(
            select(Application.id, Application.name, Application.code).where(Application.id.in_(app_ids))
        )
        app_info_map = {row.id: {"name": row.name, "code": row.code} for row in app_result}

    return PaginatedResponse(
        items=[_build_page_list_out(
            item,
            app_info_map.get(item.application_id, {}).get("name", ""),
            app_info_map.get(item.application_id, {}).get("code", ""),
        ) for item in items],
        total=result["total"],
    )
