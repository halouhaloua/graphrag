...
const sidebarMarginTop = computed(() => {
  ...
  return isMixedNav.value && !isMobile ? headerHeight : 12;
});
...
const mainStyle = computed(() => {
  ...
  if (
    ...
    if (isSideNavEffective) {
      ...
      // 100% - 侧边菜单混合宽度 - 菜单宽度 - 侧边栏左边距12px
      sidebarAndExtraWidth = `${sideCollapseWidth + sideWidth + 12}px`;
      width = `calc(100% - ${sidebarAndExtraWidth})`;
    } else {
      sidebarAndExtraWidth =
        sidebarExpandOnHovering.value && !sidebarExpandOnHover.value
          ? `${getSideCollapseWidth.value + 12}px`
          : `${getSidebarWidth.value + 12}px`;
      width = `calc(100% - ${sidebarAndExtraWidth})`;
    }
  }
  return {
...
const tabbarStyle = computed((): CSSProperties => {
  ...
  if (!isMixedNav.value || props.sidebarHidden) {
    ...
    // 设置 marginLeft，根据侧边栏是否折叠来决定，并添加 10px 间距
    marginLeft =
      (sidebarCollapse.value ? getSideCollapseWidth.value : onHoveringWidth) +
      12;

    // 设置 tabbar 的宽度，计算方式为 100% 减去侧边栏的宽度，再减去左右各 10px 的间距
    width = `calc(100% - ${sidebarCollapse.value ? getSidebarWidth.value : onHoveringWidth}px - 24px)`;
  } else {
    // 默认情况下，tabbar 的宽度为 100% 减去左右各 10px
    width = 'calc(100% - 24px)';
    marginLeft = 12;
  }
...
const contentStyle = computed((): CSSProperties => {
  ...
  return {
    marginLeft: showSidebar.value && !props.isMobile ? '12px' : 0,
    marginTop:
      fixed &&
      !isFullContent.value &&
...
const headerWrapperStyle = computed((): CSSProperties => {
  const fixed = headerFixed.value;
  const leftOffset = isMixedNav.value
    ? '12px'
    : mainStyle.value.sidebarAndExtraWidth;
  const widthCalc = isMixedNav.value
    ? 'calc(100% - 12px)'
    : mainStyle.value.width;
...
<template>
  <div class="bg-background-deep relative flex min-h-full w-full">
    <LayoutSidebar
      ...
      :is-sidebar-mixed-nav="isSidebarMixedNav"
      :margin-top="sidebarMarginTop"
      :mixed-width="sidebarMixedWidth"
      :show="showSidebar"
      :sidebar-branding="sidebarBranding"
      :theme="sidebarTheme"
    ...
    >
      ...
      <template #sidebar-footer="sidebarFooterScope">
        <slot name="sidebar-footer" v-bind="sidebarFooterScope || {}"></slot>
      </template>
    </LayoutSidebar>
    ...
    >
      <div
        ...
        class="bg-background-deep overflow-hidden transition-all duration-200"
      >
        <LayoutHeader
          v-if="headerVisible"
