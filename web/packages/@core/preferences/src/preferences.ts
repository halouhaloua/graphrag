...
class PreferenceManager {
  ...
  public async initPreferences({ namespace, overrides }: InitialOptions) {
    ...
    // 合并初始偏好设置：overrides 优先级最高
    this.initialPreferences = merge({}, overrides, defaultPreferences);

    // 加载并合并当前存储的偏好设置
    // 优先级规则：
    // - 应用名称(app.name)和logo：后端配置优先（确保品牌一致性）
    // - 其他配置：localStorage缓存优先（保留用户个性化设置）
    const cachedPreferences = this.loadCachedPreferences() || {};
    
    // 先合并：localStorage > overrides > defaults
    ...
    // 特殊处理：以下配置始终使用后端配置（如果有）
    // - 应用名称、默认首页路径、启用偏好设置：确保应用行为一致性
    // - Logo：确保品牌一致性
    if (overrides?.app?.name) {
      mergedPreference.app.name = overrides.app.name;
    }
    if (overrides?.app?.defaultHomePath) {
      mergedPreference.app.defaultHomePath = overrides.app.defaultHomePath;
    }
    if (overrides?.app?.enablePreferences !== undefined) {
      mergedPreference.app.enablePreferences = overrides.app.enablePreferences;
    }
