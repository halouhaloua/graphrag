...
import { Briefcase } from '@vben/icons';

import { ElPopover, ElSkeleton, ElTag } from 'element-plus';

import { getPostDetailApi } from '#/api/core/post';
...
const detail = ref<null | Post>(null);
...
async function loadDetail() {
  if (hasLoaded.value || !props.postId) return;

  const cached = cache.get(props.postId);
  if (cached) {
    detail.value = cached;
  ...
  loading.value = true;
  try {
    const res = await getPostDetailApi(props.postId);
    detail.value = res;
    cache.set(props.postId, res);
    hasLoaded.value = true;
  } catch (error) {
...
<template>
  ...
  >
    <template #reference>
      ...
      >
        <Briefcase v-if="showIcon" class="mr-1 h-3 w-3" />
        {{ detail.name }}
      </ElTag>
    ...
    <div v-else-if="detail" class="post-detail">
      <div class="mb-2 flex items-center gap-2">
        <Briefcase class="h-4 w-4 text-[var(--el-color-primary)]" />
      ...
      <div class="detail-grid">
        ...
        <div v-if="detail.post_type_display" class="detail-row">
          <span class="detail-label">类型</span>
          <span class="detail-value">{{ detail.post_type_display }}</span>
        </div>
        <div v-if="detail.post_level_display" class="detail-row">
          <span class="detail-label">级别</span>
          <span class="detail-value">{{ detail.post_level_display }}</span>
        </div>
        <div v-if="detail.dept_name" class="detail-row">
          <span class="detail-label">部门</span>
          <span class="detail-value">{{ detail.dept_name }}</span>
        </div>
