...
import { renameItem } from '../api/rag-file';

import { useRagFileManager } from '../composables/useRagFileManager';

const props = defineProps<{
  item: null | { id?: string; name: string; type?: string; fileType?: string; file_type?: string };
  visible: boolean;
...
const handleSubmit = async () => {
  ...
  loading.value = true;
  try {
    await renameItem(props.item.id, { name: form.value.name });
