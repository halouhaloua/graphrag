...
import { batchDelete, uploadFile } from '../api/rag-file';
...
const handleFileChange = async (event: Event) => {
  ...
  try {
    for (const file of files) {
      if (!file) continue;
      try {
        await uploadFile(file, scope, currentFolderId.value || undefined);
        successCount++;
      } catch (error) {
...
<template>
  <div class="border-border flex items-center justify-between border-b px-4 py-3">
    ...
    <div class="flex flex-shrink-0 items-center gap-2 sm:gap-3">
      ...
      <ElButton circle @click="fetchFiles(creatorId)">
        <template #icon>
